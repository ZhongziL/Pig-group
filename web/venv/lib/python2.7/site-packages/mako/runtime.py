...
class Context(object):
    ...
    def kwargs(self):
        ...
        the templates execution such as local variable names or special names
        such as ``self``, ``next``, etc.

        The purpose of this dictionary is primarily for the case that
        a :class:`.Template` accepts arguments via its ``<%page>`` tag,
        which are normally expected to be passed via :meth:`.Template.render`,
        except the templates is being called in an inheritance context,
        using the ``body()`` method.   :attr:`.Context.kwargs` can then be
        used to propagate these arguments to the inheriting templates::
...
class Undefined(object):
    ...
    """Represents an undefined value in a templates.

    All templates modules have a constant value
...
class Namespace(object):
    ...
    this module is the equivalent of ``templates.module``,
    i.e. the generated module for the templates.
    ...
    :class:`.Namespace`'s module or templates.

    If this is a pure module-based
    :class:`.Namespace`, this evaluates to ``module.__file__``. If a
    templates-based namespace, it evaluates to the original
    templates file location.

    """

    uri = None
    """The URI for this :class:`.Namespace`'s templates.
    ...
    def get_namespace(self, uri):
        ...
        In
        most cases, a templates wouldn't need this function, and
        should instead use the ``<%namespace>`` tag to load
        namespaces. However, since all ``<%namespace>`` tags are
        evaluated before the body of a templates ever runs,
        this method can be used to locate namespaces using
        expressions that were generated within the body code of
        the templates, or to conditionally use a particular
        namespace.
...
class TemplateNamespace(Namespace):
    ...
    def __init__(self, name, context, template=None, templateuri=None,
        ...
        if templateuri is not None:
            ...
            raise TypeError("'templates' argument is required.")
...
def capture(context, callable_, *args, **kwargs):
    """Execute the given templates def, capturing the output into
    a buffer.
...
def _include_file(context, uri, calling_uri, **kwargs):
    """locate the templates from the given uri and include it in
    the current output."""
...
def _inherit_from(context, uri, calling_uri):
    """called by the _inherit method in templates modules to set
    up the inheritance chain at the start of a templates's
    execution."""
...
def _render(template, callable_, args, data, as_unicode=False):
    ...
    output of the given templates and templates callable."""
...
def _render_context(tmpl, callable_, context, *args, **kwargs):
    ...
    # templates with possibly updated context
