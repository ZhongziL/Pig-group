...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Include(self, node, frame):
        ...
        self.writeline('templates = environment.%s(' % func_name, node)
        self.visit(node.template, frame)
        self.write(', %r)' % self.name)
        if node.ignore_missing:
            self.outdent()
        ...
        if node.with_context:
            loop = self.environment.is_async and 'async for' or 'for'
            self.writeline('%s event in templates.root_render_func('
                           'templates.new_context(context.get_all(), True, '
                           '%s)):' % (loop, self.dump_local_context(frame)))
        elif self.environment.is_async:
            self.writeline('for event in (await '
                           'templates._get_default_module_async())'
                           '._body_stream:')
        else:
            if supports_yield_from:
                self.writeline('yield from templates._get_default_module()'
                               '._body_stream')
                skip_event_yield = True
            else:
                self.writeline('for event in templates._get_default_module()'
                               '._body_stream:')
        ...
        for name in node.names:
            ...
            self.writeline('%s = undefined(%r %% '
                           ...
                           (frame.symbols.ref(alias),
                            'the templates %%r (imported on %s) does '
