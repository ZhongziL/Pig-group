...
templates strings, as well as templates runtime operations."""
...
class Template(object):
    ...
    """Represents a compiled templates.

    :class:`.Template` includes a reference to the original
    templates source (via the :attr:`.source` attribute)
    as well as the source code of the
    generated Python module (i.e. the :attr:`.code` attribute),
    as well as a reference to an actual Python module.

    :class:`.Template` is constructed using either a literal string
    representing the templates text, or a filename representing a filesystem
    path to a source file.

    :param text: textual templates source.  This argument is mutually
     exclusive versus the ``filename`` parameter.

    :param filename: filename of the source templates.  This argument is
    ...
    :param cache_enabled: Boolean flag which enables caching of this
     templates.  See :ref:`caching_toplevel`.
    ...
    :param format_exceptions: if ``True``, exceptions which occur during
     the render phase of this templates will be caught and
    ...
    :param future_imports: String list of names to import from `__future__`.
     ...
     into the beginning of the templates, e.g. ``futures_imports=['FOO',
    ...
    :param include_error_handler: An error handler that runs when this templates
    ...
    :param input_encoding: Encoding of the templates's source code.  Can
    ...
    :param preprocessor: Python callable which will be passed
     the full templates source before it is parsed. The return
     result of the callable will be used as the templates source
     code.

    :param lexer_cls: A :class:`.Lexer` class used to parse
     the templates.   The :class:`.Lexer` class is used by
     default.
    ...
    :param uri: string URI or other identifier for this templates.
     If not provided, the ``uri`` is generated from the filesystem
     path, or from the in-memory identity of a non-file-based
     templates. The primary usage of the ``uri`` is to provide a key
    ...
    def __init__(self,
        ...
        if text is not None:
            ...
            # if templates filename and a module directory, load
    ...
    def _compile_from_file(self, path, filename):
        if path is not None:
            ...
            # templates filename and no module directory, compile code
            # in memory
            data = util.read_file(filename)
    ...
    @property
    def source(self):
        """Return the templates source code for this :class:`.Template`."""
    ...
    def render(self, *args, **data):
        """Render the output of this templates as a string.

        If the templates specifies an output encoding, the string
        will be encoded accordingly, else the output is raw (raw
        output uses `cStringIO` and can't handle multibyte
        characters). A :class:`.Context` object is created corresponding
        to the given data. Arguments that are explicitly declared
        by this templates's internal rendering method are also
    ...
    def render_unicode(self, *args, **data):
        """Render the output of this templates as a unicode object."""
    ...
    def get_def(self, name):
        """Return a def of this templates as a :class:`.DefTemplate`."""

        return DefTemplate(self, getattr(self.module, "render_%s" % name))

    def list_defs(self):
        """return a list of defs in the templates.
...
class ModuleTemplate(Template):
    ...
    """A Template which is constructed given an existing Python module.
        ...
        t = Template("this is a templates")
...
class ModuleInfo(object):
    ...
    memory, provides reverse lookups of templates source, module
